    ...
    public MyContactEdit(final String id, final UserProfile userProfile) {
		...
		AjaxFallbackButton submitButton = new AjaxFallbackButton("submit", new ResourceModel("button.save.changes"), form) {
			...
			protected void onError(AjaxRequestTarget target, Form form) {
				...
				if(!email.isValid()) {
					email.add(new AttributeAppender("class", new Model("invalid"), " "));
					emailFeedback.setModel(new ResourceModel("EmailAddressValidator"));
					target.addComponent(email);
					target.addComponent(emailFeedback);
				}
				if(!homepage.isValid()) {
					homepage.add(new AttributeAppender("class", new Model("invalid"), " "));
					homepageFeedback.setModel(new ResourceModel("UrlValidator"));
					target.addComponent(homepage);
					target.addComponent(homepageFeedback);
				}
				if(!facsimile.isValid()) {
					facsimile.add(new AttributeAppender("class", new Model("invalid"), " "));
					facsimileFeedback.setModel(new ResourceModel("PhoneNumberValidator"));
					target.addComponent(facsimile);
					target.addComponent(facsimileFeedback);
				}
				
				if(!workphone.isValid()) {
					workphone.add(new AttributeAppender("class", new Model("invalid"), " "));
					workphoneFeedback.setModel(new ResourceModel("PhoneNumberValidator"));
					target.addComponent(workphone);
					target.addComponent(workphoneFeedback);
				}
				if(!homephone.isValid()) {
					homephone.add(new AttributeAppender("class", new Model("invalid"), " "));
					homephoneFeedback.setModel(new ResourceModel("PhoneNumberValidator"));
					target.addComponent(homephone);
					target.addComponent(homephoneFeedback);
				}
				if(!mobilephone.isValid()) {
					mobilephone.add(new AttributeAppender("class", new Model("invalid"), " "));
					mobilephoneFeedback.setModel(new ResourceModel("PhoneNumberValidator"));
